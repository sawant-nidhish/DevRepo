...
export class StockDetailsComponent {
  ...
  checkInWatchList(symbol:string){
    let response=this.http.get(env+`/api/watchList?symbol=${symbol}`).pipe(
    catchError(error => {
  ...
  response.subscribe(data=>{
   ...
   async addToWatchList(stock:any){
    const response= this.http.post(env+`/api/watchList`,stock).pipe(
      catchError(error => {
        console.error('Error fetching company data:', error);
        throw error;
      }),
  ...
  checkInPortfolio(symbol:string){
    let response=this.http.get(env+`/api/portfolio?symbol=${symbol}`).pipe(
    catchError(error => {
      console.error('Error fetching company data:', error);
      throw error;
    })
...
export class NgbdModalContent {
  ...
  async buyStock(stock:any,buy:string){
    const response= this.http.post(env+`/api/portfolio?buy=${buy}`,stock).pipe(
      catchError(error => {
        console.error('Error fetching company data:', error);
        throw error;
      }),
