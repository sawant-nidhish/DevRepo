...
function getStock(event){
  ...
  search_bar=document.querySelector('#ticker');
  ...
  fetch(`/company?ticker=${ticker}`)
    ...
    .then(function(json){
      if(isEmpty(json)){
        ...
        document.querySelector('.allTabs').style.display='none';
        ...
        document.querySelector('.allTabs').style.display='block';
    ...
    .then(function(json){
      if(isEmpty(json)){
        ...
        for(var i=0; i<5; i++){
          ...
          let news_box=`<div>
                        ${i}
                        <img src=${img_url} style="display:inline-block; height:2em;width:2em;">
                        <p id="news_heading">${heading}</p>
                        <p id="news_date">${date}</p>
                        <p id="news_date"><a href="${url}" target="_blank">See Original Post</a></p>
                      </div>`;
